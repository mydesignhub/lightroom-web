...
const KNOWLEDGE_BASE = [
    // --- EVERYDAY CONVERSATION & AI IDENTITY ---
    ...
    // --- PHOTOGRAPHY BASICS & CAMERA CONCEPTS ---
    ...
    // --- SPECIFIC EDITING SCENARIOS & TRENDS ---
...
const findAIResponse = (input) => {
    ...
    // Improved matching logic: searches whole string against keys without strict boundaries
...
const BASE_PRESETS_DATA = {
    ...
    ...generateVariations("era_classic_bw", "Eras & Vintage", "B&W Noir", { basic: { Exposure: -0.15, Contrast: 35, Highlights: -20, Shadows: 15, Whites: -10, Blacks: -25, Saturation: -100, Clarity: 15 }, curveMaster: [{x:0,y:0}, {x:25,y:15}, {x:50,y:45}, {x:75,y:80}, {x:100,y:100}] }, 3),
...
const PhotoLab = ({ isDarkMode }) => {
  ...
  const handleCurvePointerDown = (e) => {
      ...
      for (let i = 0; i < points.length; i++) {
          ...
          // Massive hit area for endpoints (35) making them super easy to grab
  ...
  const handleCurvePointerMove = (e) => {
      ...
      if (draggingPointIndex === 0) {
          // Bottom-Left Endpoint: Left edge or Bottom edge
          ...
          // Top-Right Endpoint: Right edge or Top edge
  ...
  // True Bezier interpolation matching standard photo tools
  const renderSmoothCurve = () => {
      ...
      for (let i = 0; i < activePoints.length - 1; i++) {
          ...
          // Invisible points extending 180 degrees outwards to keep endpoints smooth
  ...
  // Re-evaluating Y matching the Bezier behavior for SVG filters
  const evaluateSplineForFilter = (points, targetX) => {
      ...
      // Invisible endpoints mapped for the math filter
  ...
  const getChannelTable = (channelName) => {
      ...
      for(let i = 0; i <= numSamples; i++) {
          ...
          const masterOut = evaluateSplineForFilter(settings.curveMaster, basicOut * 100) / 100;
          const finalOut = evaluateSplineForFilter(settings[`curve${channelName}`], masterOut * 100) / 100;
